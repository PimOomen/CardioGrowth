function h = plotBullseyeSegment(c, cLim, cMap, cBarTitle,...
                             shadingSwitch, fName, txtFormat, visSwitch,...
                             fSize, lSize, plotParsg, ischemicCompartments)
...
h = figure('Visible', visSwitch); hold on

% Patches - flip x axis as theta = 0 is in the septum (on the right)
for i = 1:length(c)
    hp(i) = patch(-segments(i).xPatch, segments(i).yPatch, c(i,:), 'EdgeColor', 'None');   
end
    

% Contour lines
plot(contours.X1,contours.Y1, 'k', 'LineWidth', lSize);
plot(contours.X2,contours.Y2, 'k', 'LineWidth', lSize);
plot(contours.X3,contours.Y3, 'k', 'LineWidth', lSize);
plot(contours.X0,contours.Y0, 'k', 'LineWidth', lSize);
plot(contours.X4,contours.Y4, 'k', 'LineWidth', lSize);
plot(contours.X5,contours.Y5, 'k', 'LineWidth', lSize);
plot(contours.X6,contours.Y6, 'k', 'LineWidth', lSize);
plot(contours.X7,contours.Y7, 'k', 'LineWidth', lSize);
plot(contours.X8,contours.Y8, 'k', 'LineWidth', lSize);
plot(contours.X9,contours.Y9, 'k', 'LineWidth', lSize);
plot(contours.X10,contours.Y10, 'k', 'LineWidth', lSize);
plot(contours.X11,contours.Y11, 'k', 'LineWidth', lSize);
plot(contours.X12,contours.Y12, 'k', 'LineWidth', lSize);
plot(contours.X13,contours.Y13, 'k', 'LineWidth', lSize);


text(0,128,'Anterior','fontsize',fSize,'fontweight','b','HorizontalAlignment','center', 'Color', plotParsg.fgColour)
text(-128,0,'Septal','fontsize',fSize,'Rotation',90,'fontweight','b','HorizontalAlignment','center', 'Color', plotParsg.fgColour)
text(128,0,'Lateral','fontsize',fSize,'Rotation',270,'fontweight','b','HorizontalAlignment','center', 'Color', plotParsg.fgColour)
text(0,-128,'Posterior','fontsize',fSize,'fontweight','b','HorizontalAlignment','center', 'Color', plotParsg.fgColour)
...
if shadingSwitch
    ...
    for i = ischemicCompartments
        hp(i).FaceColor = [0.2 0.2 0.2];
    end
end

axis([-120 120 -135 135]);
axis off equal;
% set(gcf,'Color','white')
...
set(gcf, 'Position', [pos(1) pos(2) pos(3) pos(3)], 'color', plotParsg.bgColour)
...
for i = 1:length(c)
    ...
    if ~shadingSwitch
        text(-segments(i).xCenter, segments(i).yCenter, num2str(i, txtFormat),...
                'FontSize', fSize, 'Color', [0 0 0],'HorizontalAlignment','center')
...
% 
%     if ~isempty(fName)
%         print(hc, '-dpdf', strcat(fName, 'Cbar'))
%     end
% end