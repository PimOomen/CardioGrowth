...
SBV = 310;         % Stresses blood volume for Vernooy (fit 5) - acute change x0.95
Ras = 6.87;         % Systemic arterial resistance (SVR in CMW paper)
...
A = 0.288;                % Linear ED component [mmHg]
B = 0.214;                % Exponential ED component [1/mL] 
LVEes = 23.4;               % End-systolic elastance of LV [mmHg/mL]
V0 = 6.4;                 % End-systolic elastance of LV [mmHg/mL]
...
solverSettings.iterMax = 100;       % Maximum number of iterationsclear all
...
tTotal = 18;              % Total simulation time [days]
...
SBVg = [310; repmat(412, [Ng-1 1])];
Rasg = [6.87; repmat(12.0, [Ng-1 1])];
...
ratioval = Rasg(2)/maxpdivco(2);
Rasg(3:end) = maxpdivco(3:end)*ratioval; 
HRg = [100 109 100.4326 100.0785 100.0234 100.0091 100.0042 100.0022 100.0013 100.0008 100.0005 100.0003 100.0002 100.0002 100.0001 100.0001 100.0001 100.0001 100 100]; %heart rate from WorldCongress2018SlidesforJeff.xlsx 
MVBRg = repmat(resistances(7), [Ng 1]);

% Mechanical activation - columns are time rows are compartments.
% Irrelevant for PO, used for dyssynchrony simulations
tActivationg = zeros(1, Ng);          
Ischemicg = zeros(1,Ng);
